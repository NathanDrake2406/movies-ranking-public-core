...
import { computeKvTtl, kvGet, kvSet } from "./kv";
...
describe("computeKvTtl", () => {
  ...
  describe("with exact release date", () => {
    it("returns 3 hours for films released less than 4 months ago", () => {
      expect(computeKvTtl("2025-12-01", undefined, now)).toBe(THREE_HOURS); // ~2 months ago
      expect(computeKvTtl("2026-01-15", undefined, now)).toBe(THREE_HOURS); // ~3 weeks ago
      expect(computeKvTtl("2025-10-10", undefined, now)).toBe(THREE_HOURS); // ~4 months ago (under)
    });
  ...
  describe("year-only fallback", () => {
    it("returns 3 hours for current-year films", () => {
      expect(computeKvTtl(undefined, "2026", now)).toBe(THREE_HOURS); // age 0
    });
...
// ─── kvGet / kvSet (mocked shared Redis client) ─────────────────────────────

const mockGet = vi.fn();
const mockSet = vi.fn();
const mockClient = { get: mockGet, set: mockSet };
...
describe("kvGet", () => {
  beforeEach(() => {
    mockGet.mockReset();
    vi.mocked(getRedisClient).mockReturnValue(mockClient as never);
  });
  ...
  it("returns null when Redis client is disabled", async () => {
  ...
  it("works when client is available (env var fallback handled by redis.ts)", async () => {
    mockGet.mockResolvedValue({ ...samplePayload, _v: 2 });
    const result = await kvGet("tt0111161");
    expect(result).toEqual(samplePayload);
    expect(result).not.toHaveProperty("_v");
...
describe("kvSet", () => {
  ...
  it("writes with 3-hour TTL for recent films", async () => {
    mockSet.mockResolvedValue(undefined);
