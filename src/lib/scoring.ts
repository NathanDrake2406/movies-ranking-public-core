...
export type TierWeights = {
  ...
  readonly cinephile: number;
  readonly mainstream: number;
};

export type ScoringLens = "default" | "balanced" | "mainstream";

/** Adding a lens = adding one entry here. */
export const LENS_WEIGHTS: ReadonlyMap<ScoringLens, TierWeights> = new Map([
  ["default", { critic: 0.5, cinephile: 0.3, mainstream: 0.2 }],
  ["balanced", { critic: 1 / 3, cinephile: 1 / 3, mainstream: 1 / 3 }],
  ["mainstream", { critic: 0.2, cinephile: 0.3, mainstream: 0.5 }],
]);

const DEFAULT_WEIGHTS = LENS_WEIGHTS.get("default")!;
...
export function computeOverallScore(
  ...
  // Resolve tier weights for the requested lens
  const tw = (lens && LENS_WEIGHTS.get(lens)) ?? DEFAULT_WEIGHTS;

  // Build tier list with weights, redistributing when tiers are null
  const tiers: Array<{ score: number; weight: number }> = [];
  tiers.push({ score: criticScore, weight: tw.critic });
  if (cinephileScore != null) {
    tiers.push({ score: cinephileScore, weight: tw.cinephile });
  }
  if (mainstreamScore != null) {
    tiers.push({ score: mainstreamScore, weight: tw.mainstream });
  }
