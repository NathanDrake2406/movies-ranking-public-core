...
function makeSource(
  ...
  const base: SourceScore = { source, label: source, normalized };
...
describe("scoring", () => {
  ...
  describe("computeOverallScore", () => {
    ...
    it("backward compatible: no lens argument = default behavior", () => {
      const sources = makeAllSources();
      const withoutLens = computeOverallScore(sources);
      const withDefault = computeOverallScore(sources, "default");
      expect(withoutLens!.score).toBeCloseTo(withDefault!.score, 6);
    });
  });

  describe("scoring lenses", () => {
    it("LENS_WEIGHTS contains all three lenses", () => {
      expect(LENS_WEIGHTS.has("default")).toBe(true);
      expect(LENS_WEIGHTS.has("balanced")).toBe(true);
      expect(LENS_WEIGHTS.has("mainstream")).toBe(true);
    });

    it("each lens sums to ~1.0", () => {
      for (const [, weights] of LENS_WEIGHTS) {
        const sum = weights.critic + weights.cinephile + weights.mainstream;
        expect(sum).toBeCloseTo(1.0, 6);
      }
    });

    it("different lenses produce different scores", () => {
      const sources = makeAllSources();
      const defaultResult = computeOverallScore(sources, "default")!;
      const balancedResult = computeOverallScore(sources, "balanced")!;
      const mainstreamResult = computeOverallScore(sources, "mainstream")!;

      // All three should produce non-null results
      expect(defaultResult).not.toBeNull();
      expect(balancedResult).not.toBeNull();
      expect(mainstreamResult).not.toBeNull();

      // They should differ (unless all tiers happen to be identical)
      const scores = new Set([
        defaultResult.score.toFixed(2),
        balancedResult.score.toFixed(2),
        mainstreamResult.score.toFixed(2),
      ]);
      expect(scores.size).toBeGreaterThanOrEqual(2);
    });

    it("mainstream lens weights audience scores more heavily", () => {
      // High mainstream, low critic/cinephile → mainstream lens should score higher
      const sources = makeAllSources({
        rotten_tomatoes_top: 60,
        metacritic: 55,
        rotten_tomatoes_all: 58,
        allocine_press: 55,
        letterboxd: 60,
        rotten_tomatoes_audience: 90,
        imdb: 92,
        douban: 88,
        allocine_user: 85,
      });
      ...
      expect(mainstreamResult.score).toBeGreaterThan(defaultResult.score);
    });

    it("balanced lens gives equal weight to all tiers", () => {
      // Extreme tier separation: critics=90, cinephile=60, mainstream=60
      const sources = makeAllSources({
        rotten_tomatoes_top: 92,
        metacritic: 90,
        rotten_tomatoes_all: 90,
        allocine_press: 88,
        letterboxd: 60,
        rotten_tomatoes_audience: 60,
        imdb: 60,
        douban: 60,
        allocine_user: 60,
      });
      ...
      // Default gives critics 50% weight, so it should score higher for critic-heavy films
      expect(defaultResult.score).toBeGreaterThan(balancedResult.score);
    });

    it("coverage and disagreement are identical across lenses", () => {
      const sources = makeAllSources();
      const defaultResult = computeOverallScore(sources, "default")!;
      const balancedResult = computeOverallScore(sources, "balanced")!;
      const mainstreamResult = computeOverallScore(sources, "mainstream")!;

      // Coverage and disagreement don't depend on tier weights
      expect(balancedResult.coverage).toBeCloseTo(defaultResult.coverage, 6);
      expect(mainstreamResult.coverage).toBeCloseTo(defaultResult.coverage, 6);
      expect(balancedResult.disagreement).toBeCloseTo(
        defaultResult.disagreement,
        6,
      );
      expect(mainstreamResult.disagreement).toBeCloseTo(
        defaultResult.disagreement,
        6,
      );
    });
