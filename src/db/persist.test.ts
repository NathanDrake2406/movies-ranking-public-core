...
describe("persistScores", () => {
  ...
  it("does nothing when getDb returns null (graceful degradation)", async () => {
    const { getDbWs } = await import("./client");
    vi.mocked(getDbWs).mockReturnValue(null);
  ...
  it("calls transaction with movie upsert, score delete, and score insert", async () => {
    ...
    const { getDbWs } = await import("./client");
    vi.mocked(getDbWs).mockReturnValue(mockDb as never);
    ...
    // No select needed — payload already has rotten_tomatoes_top
  ...
  it("deletes stale scores before inserting (prevents orphan rows)", async () => {
    ...
    // (no select because payload already has rotten_tomatoes_top)
    expect(callOrder).toEqual(["insert", "delete", "insert"]);
  });

  it("preserves existing rotten_tomatoes_top when absent from new scrape", async () => {
    const existingTopRow = {
      imdbId: "tt1234567",
      ...
      rawScale: "0-100",
      count: 52,
      url: "https://www.rottentomatoes.com/m/test",
      error: null,
      fromFallback: false,
    ...
    const mockDeleteWhere = vi.fn().mockResolvedValue(undefined);
    const mockDelete = vi.fn().mockReturnValue({ where: mockDeleteWhere });
    const mockSelectWhere = vi.fn().mockResolvedValue([existingTopRow]);
    const mockSelectFrom = vi.fn().mockReturnValue({ where: mockSelectWhere });
    const mockSelect = vi.fn().mockReturnValue({ from: mockSelectFrom });
    const mockTx = {
      insert: mockInsert,
    ...
    // Payload WITHOUT rotten_tomatoes_top (simulates RT no longer providing it)
    const payload = makePayload({
      sources: [
        { source: "imdb", label: "IMDb", normalized: 85 },
        { source: "rotten_tomatoes_all", label: "RT All", normalized: 88 },
    ...
    // Should query for existing rotten_tomatoes_top
    expect(mockSelect).toHaveBeenCalledOnce();

    // Bulk score insert (2nd insert call) should include the preserved row
    const insertedScores = mockInsertValues.mock.calls[1]?.[0] as Array<{
      source: string;
    }>;
    const sources = insertedScores.map((r) => r.source);
    expect(sources).toContain("rotten_tomatoes_top");
  });
