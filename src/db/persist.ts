...
import { WEIGHTED_SOURCE_KEYS, computeOverallScore } from "@/lib/scoring";
import { log } from "@/lib/logger";
import { CURRENT_SCORE_VERSION as SCORE_VERSION } from "./score-version";

/** Bump when weights or scoring algorithm change — enables re-derivation queries. */
export const CURRENT_SCORE_VERSION = SCORE_VERSION;
...
export function payloadToMovieRow(
  ...
  return {
    ...
    directorPhotos: movie.directorPhotos?.map((p) => p ?? "") ?? null,
    writers: movie.writers ?? null,
    cinematographer: movie.cinematographer ?? null,
    editor: movie.editor ?? null,
    composer: movie.composer ?? null,
    castMembers: movie.cast ?? null,
    watchProviderIds: movie.allStreamProviderIds?.length
      ? movie.allStreamProviderIds
      : (movie.watchProviders
          ?.filter((wp) => wp.type === "stream")
          .map((wp) => wp.id) ?? null),
    themes: payload.themes ?? null,
    consensus: payload.consensus ?? null,
    imdbSummary: payload.imdbSummary ?? null,
    overallScore: overall?.score ?? null,
    scoreBalanced: computeOverallScore(sources, "balanced")?.score ?? null,
    scoreMainstream: computeOverallScore(sources, "mainstream")?.score ?? null,
  ...
  try {
    await db.transaction(async (tx) => {
      if (isBackfill) {
        ...
        await tx
          ...
          .onConflictDoUpdate({
            ...
            set: {
              ...
              directorPhotos: movieRow.directorPhotos,
              writers: movieRow.writers,
              cinematographer: movieRow.cinematographer,
              editor: movieRow.editor,
              composer: movieRow.composer,
              castMembers: movieRow.castMembers,
              watchProviderIds: movieRow.watchProviderIds,
              themes: movieRow.themes,
              consensus: movieRow.consensus,
              imdbSummary: movieRow.imdbSummary,
              overallScore: movieRow.overallScore,
              scoreBalanced: movieRow.scoreBalanced,
              scoreMainstream: movieRow.scoreMainstream,
        ...
        // RT removed top-critics averageRating from their embedded JSON (early 2026).
        // Without this, every re-scrape permanently deletes the historical row.
        // Only preserve rotten_tomatoes_top — all other sources should update normally.
        const newSources = new Set(scoreRows.map((r) => r.source));
        let preserved: NewScore[] = [];
        if (!newSources.has("rotten_tomatoes_top")) {
          const existing = await tx
            .select()
            .from(scores)
            .where(
              and(
                eq(scores.imdbId, payload.movie.imdbId),
                eq(scores.source, "rotten_tomatoes_top"),
                isNotNull(scores.normalized),
              ),
            );
          preserved = existing.map((r) => ({
            imdbId: r.imdbId,
            source: r.source,
            label: r.label,
            ...
            url: r.url,
            error: r.error,
            fromFallback: r.fromFallback ?? false,
            updatedAt: r.updatedAt,
          }));
        }

        // Replace all scores: delete stale rows then insert current set + preserved.
        // This prevents orphans when the source set changes (e.g., split RT → OMDB fallback)
        // while retaining historical scores for sources that upstream no longer provides.
        await tx.delete(scores).where(eq(scores.imdbId, payload.movie.imdbId));
        const allRows = [...scoreRows, ...preserved];
        if (allRows.length > 0) {
          await tx.insert(scores).values(allRows);
        }

        if (preserved.length > 0) {
          log.info("scores_preserved", {
            imdbId: payload.movie.imdbId,
            sources: preserved.map((r) => r.source),
          });
        }
      }
    });
